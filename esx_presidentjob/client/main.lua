...
AddEventHandler('playerSpawned', function(spawn)
	...
	TriggerServerEvent('esx_presidentjob:requestPlayerData', 'playerSpawned')
end)

RegisterNetEvent('esx_phone:loaded')
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	
	TriggerEvent('esx_phone:addContact', 'President', 'president', 'special', false)
...
AddEventHandler('esx_presidentjob:hasEnteredMarker', function(zone)

	if zone == 'CloakRoom' then
		if PlayerData.job.name ~= nil and PlayerData.job.name == 'gouvernement' then
			SendNUIMessage({
				showControls = true,
				controls     = 'cloakroom'
			})
...
if zone == 'HelicopterLandingPadSpawner1' then
		...
		if SpawnedVehicles['supervolito'] ~= nil and not DoesEntityExist(GetPedInVehicleSeat(SpawnedVehicles['supervolito'], -1)) then
			DeleteVehicle(SpawnedVehicles['supervolito'])
			SpawnedVehicles['supervolito'] = nil
		end

		Citizen.CreateThread(function()

			local coords      = Config.Zones.HelicopterLandingPad1.Pos
			local vehicleModel = GetHashKey('supervolito')
		...
		while not HasModelLoaded(vehicleModel) do
			...
			if not IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 10.0) then
				SpawnedVehicles['supervolito'] = CreateVehicle(vehicleModel, coords.x, coords.y, coords.z, 0.0, true, false)
				SetVehicleModKit(SpawnedVehicles['supervolito'], 0)
				SetVehicleLivery(SpawnedVehicles['supervolito'], 0)
				SetVehicleHasBeenOwnedByPlayer(SpawnedVehicles['supervolito'],  true)
				SetEntityAsMissionEntity(SpawnedVehicles['supervolito'],  true,  true)
				local id = NetworkGetNetworkIdFromEntity(SpawnedVehicles['supervolito'])
...
AddEventHandler('esx_presidentjob:hasExitedMarker', function(zone)
	SendNUIMessage({
		showControls = false,
		showMenu     = false,
	})
...
RegisterNetEvent('esx_presidentjob:handcuff')
AddEventHandler('esx_presidentjob:handcuff', function()
...
RegisterNetEvent('esx_presidentjob:putInVehicle')
AddEventHandler('esx_presidentjob:putInVehicle', function()
...
RegisterNetEvent('esx_presidentjob:confiscatePlayerWeapon')
AddEventHandler('esx_presidentjob:confiscatePlayerWeapon', function(weaponName)
	local playerPed = GetPlayerPed(-1)
	RemoveWeaponFromPed(playerPed,  GetHashKey(weaponName))
end)

RegisterNetEvent('esx_presidentjob:requestPlayerWeapons')
AddEventHandler('esx_presidentjob:requestPlayerWeapons', function(playerId, reason)
	...
	TriggerServerEvent('esx_presidentjob:responsePlayerWeapons', weapons, playerId, reason)

end)

RegisterNetEvent('esx_presidentjob:responsePlayerPositions')
AddEventHandler('esx_presidentjob:responsePlayerPositions', function(positions, reason)
    ...
    if closestPlayer ~= -1 then
    	...
    	TriggerServerEvent('esx_presidentjob:requestOtherPlayerData', closestPlayer, 'identity_card')
		end
    ...
    if closestPlayer ~= -1 then
    	PoliceMenuTargetPlayerId = closestPlayer;
    	TriggerServerEvent('esx_presidentjob:requestOtherPlayerData', closestPlayer, 'body_search')
		end
    ...
    if closestPlayer ~= -1 then
    	PoliceMenuTargetPlayerId = closestPlayer;
    	TriggerServerEvent('esx_presidentjob:handcuff', closestPlayer);
		end
    ...
    if closestPlayer ~= -1 then
    	PoliceMenuTargetPlayerId = closestPlayer;
    	TriggerServerEvent('esx_presidentjob:putInVehicle', closestPlayer);
		end
    ...
    if closestPlayer ~= -1 then
    	PoliceMenuTargetPlayerId = closestPlayer;
    	TriggerServerEvent('esx_presidentjob:applyFine', closestPlayer, CurrentFine);
		end
...
RegisterNetEvent('esx_presidentjob:responsePlayerData')
AddEventHandler('esx_presidentjob:responsePlayerData', function(data, reason)
	PlayerData = data
end)

RegisterNetEvent('esx_presidentjob:responseOtherPlayerData')
AddEventHandler('esx_presidentjob:responseOtherPlayerData', function(data, reason)
...
RegisterNetEvent('esx_presidentjob:responseFineData')
AddEventHandler('esx_presidentjob:responseFineData', function(data)
...
RegisterNetEvent('esx_presidentjob:responseFineList')
AddEventHandler('esx_presidentjob:responseFineList', function(fines)
...
RegisterNetEvent('esx_presidentjob:hasPayedFine')
AddEventHandler('esx_presidentjob:hasPayedFine', function(playerName, amount)
...
RegisterNUICallback('select', function(data, cb)
		...
		if data.menu == 'citizen_interaction' then
			...
			if data.val == 'identity_card' then
				TriggerServerEvent('esx_presidentjob:requestPlayerPositions', 'identity_card')
			end

			if data.val == 'body_search' then
				TriggerServerEvent('esx_presidentjob:requestPlayerPositions', 'body_search')
			end

			if data.val == 'handcuff' then
				TriggerServerEvent('esx_presidentjob:requestPlayerPositions', 'handcuff')
			end

			if data.val == 'put_in_vehicle' then
				TriggerServerEvent('esx_presidentjob:requestPlayerPositions', 'put_in_vehicle')
			end
		...
		if data.menu == 'body_search' then
			...
			if data.type == 'black_money' then
				local playerServerId = GetPlayerServerId(PlayerId())
				TriggerServerEvent('esx_presidentjob:confiscatePlayerBlackMoney', PoliceMenuTargetPlayerId, data.val)
			end

			if data.type == 'weapon' then
				local playerPed = GetPlayerPed(-1)
				TriggerServerEvent('esx_presidentjob:confiscatePlayerWeapon', PoliceMenuTargetPlayerId, data.val)
				GiveWeaponToPed(playerPed,  GetHashKey(data.val),  0,  false,  false)
			end

			if data.type == 'inventory_item' then
				local playerServerId = GetPlayerServerId(PlayerId())
				TriggerServerEvent('esx_presidentjob:confiscatePlayerInventoryItem', PoliceMenuTargetPlayerId, data.val, tonumber(data.count))
				TriggerServerEvent('esx_presidentjob:addPlayerInventoryItem',        playerServerId,           data.val, tonumber(data.count))
			end

		end

		if data.menu == 'fine' then
			TriggerServerEvent('esx_presidentjob:requestFineData', data.val)
		end

		if data.menu == 'fine_data' then
			CurrentFine = data.val
			TriggerServerEvent('esx_presidentjob:requestPlayerPositions', 'fine_data')
		end

		if data.menu == 'fine_list' then
			TriggerServerEvent('esx_presidentjob:requestPayFine', data.val, data.count, GetPlayerName(PlayerId()))
		end
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.Zones) do
			...
			if(PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' and v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
	...
	while true do
		...
		if(PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement') then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_presidentjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_presidentjob:hasExitedMarker', lastZone)
			end
...
-- Create blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(-429.121, 1111.39,  327,82)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' and IsControlPressed(0, Keys['F6']) and (GetGameTimer() - GUI.Time) > 300 then
		...
		if IsControlPressed(0, Keys['F7']) and (GetGameTimer() - GUI.Time) > 300 then
			...
			TriggerServerEvent('esx_presidentjob:requestFineList')
